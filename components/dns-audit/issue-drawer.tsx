...
function CodeText({ value }: { value: string | null }) {
  if (!value) return <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13 }}>Not specified</span>
  return (
    <code style={{
      display: 'block',
      padding: '8px 12px',
      background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)',
      borderRadius: 8, fontSize: 12, color: '#86EFAC', lineHeight: 1.7,
...
export function IssueDrawer({ issue, onClose, onUpdate }: IssueDrawerProps) {
  ...
  const sevMeta = SEVERITY_META[issue.severity] ?? { label: issue.severity, color: '#94A3B8' }
  const statusMeta = STATUS_OPTIONS.find(s => s.value === status) ?? STATUS_OPTIONS[0]
  const issueTypeMeta = ISSUE_TYPE_META[issue.issueType] ?? { label: issue.issueType, color: '#94A3B8' }
  ...
  async function handleStatusChange(newStatus: string) {
    ...
    try {
      await fetch(`/api/dns-audit/issues/${issue.id}`, {
        method: 'PATCH',
  ...
  async function loadEvidence() {
    ...
    try {
      const res = await fetch(`/api/dns-audit/issues/${issue.id}/evidence`)
      if (res.ok) {
  ...
  async function handleDeleteEvidence(evidenceId: string) {
    try {
      await fetch(`/api/dns-audit/issues/${issue.id}/evidence/${evidenceId}`, { method: 'DELETE' })
  ...
  async function loadComments() {
    ...
    try {
      const res = await fetch(`/api/dns-audit/issues/${issue.id}/comments`)
      if (res.ok) {
        const data = await res.json()
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
                ...
                <span style={{
                  ...
                  background: `${issueTypeMeta.color}14`, border: `1px solid ${issueTypeMeta.color}35`,
                  color: issueTypeMeta.color, textTransform: 'uppercase', letterSpacing: '0.04em',
                }}>
                  {issueTypeMeta.label}
                </span>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
            ...
            <div>
              <Section title="Overview">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
                  <FieldValue label="Affected Record">
                    {issue.affectedRecord ?? <span style={{ color: 'rgba(255,255,255,0.25)' }}>Not specified</span>}
                  </FieldValue>
                  <FieldValue label="Record Type">
                    {issue.recordType ? (
                      <code style={{ fontSize: 12, background: 'rgba(255,255,255,0.08)', padding: '2px 7px', borderRadius: 4, color: '#7DD3FC' }}>
                        {issue.recordType}
                      </code>
                  ...
                  <FieldValue label="Due Date">
                    {issue.dueDate ? (
                      <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: new Date(issue.dueDate) < new Date() ? '#F97316' : '#E2E8F0' }}>
                        <Calendar size={13} />
                        {new Date(issue.dueDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                      </span>
              ...
              <Section title="Current Value">
                <CodeText value={issue.currentValue} />
              </Section>

              <Section title="Expected Value">
                <CodeText value={issue.expectedValue} />
              </Section>

              <Section title="Description">
                <PreText value={issue.description} />
              </Section>

              <Section title="Risk Details">
                <PreText value={issue.riskDetails} />
              </Section>

              <Section title="Remediation Guidance">
                <PreText value={issue.remediation} />
              </Section>
          ...
          {activeTab === 'evidence' && (
            <div>
              ...
              >
                <Upload size={24} color={dragOver ? '#10B981' : 'rgba(255,255,255,0.3)'} style={{ margin: '0 auto 10px' }} />
              ...
              {uploading && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.08)' }}>
                    <div style={{ height: '100%', borderRadius: 2, background: 'linear-gradient(90deg, #10B981, #06B6D4)', width: `${uploadProgress}%`, transition: 'width 0.3s ease' }} />
                  </div>
                </div>
              )}
                    ...
                    <div key={c.id} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(16,185,129,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#6EE7B7' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: newComment.trim() ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.04)',
                      border: `1px solid ${newComment.trim() ? 'rgba(16,185,129,0.35)' : 'rgba(255,255,255,0.08)'}`,
                      color: newComment.trim() ? '#6EE7B7' : 'rgba(255,255,255,0.25)',
