...
import { X, RefreshCw, ShieldOff } from 'lucide-react'
...
export function NewAuditDialog({ onClose, onSuccess }: NewAuditDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const res = await fetch('/api/firewall-audit/audits', {
        method: 'POST',
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ShieldOff size={15} color="#3B82F6" />
            </div>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#F1F5F9' }}>New Firewall Audit</div>
              <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.4)' }}>Create a new firewall audit record</div>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Q3 2024 Perimeter Firewall Audit"
                style={inputStyle}
                autoFocus
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 4 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: submitting ? 'rgba(59,130,246,0.4)' : 'linear-gradient(135deg, #3B82F6, #2563EB)',
