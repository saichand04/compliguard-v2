/**
 * app/api/integrations/sentinel/route.ts
 * GET + POST + DELETE for Azure Sentinel SIEM integration config.
 * Stored as type='azure' with subType='sentinel'.
 */
...
const INTEGRATION_NAME = 'Azure Sentinel SIEM'
const INTEGRATION_SUBTYPE = 'sentinel'

type IntegrationType = 'azure'
const INTEGRATION_TYPE: IntegrationType = 'azure'

async function getSentinelRow(orgId: string) {
  const rows = await db
    .select()
    .from(integrations)
    .where(
...
// GET /api/integrations/sentinel
export async function GET(req: NextRequest) {
  const session = await requireAuth(req)
  if (!session) return ApiErrors.unauthorized()

  const row = await getSentinelRow(session.orgId!)
...
// POST /api/integrations/sentinel
export async function POST(req: NextRequest) {
  ...
  if (!body.tenantId || !body.clientId || !body.clientSecret || !body.subscriptionId || !body.resourceGroup || !body.workspaceName) {
    return ApiErrors.badRequest('tenantId, clientId, clientSecret, subscriptionId, resourceGroup, and workspaceName are required')
  }
  ...
  const existing = await getSentinelRow(session.orgId!)
...
// DELETE /api/integrations/sentinel
...
// Helper: get decrypted sentinel config
export async function getSentinelConfig(orgId: string): Promise<{
  tenantId: string
  clientId: string
  clientSecret: string
  subscriptionId: string
  ...
  const row = await getSentinelRow(orgId)
  if (!row) return null
  ...
  return {
    ...
    resourceGroup: String(cfg.resourceGroup ?? ''),
    workspaceName: String(cfg.workspaceName ?? ''),
  }
}
